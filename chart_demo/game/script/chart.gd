...
export(Texture) var dot_texture = preload("res://texture/graph-plot-white.png")
...
func _input(event):
	if event is InputEventMouseMotion:
		...
		if event.position.distance_to(center_point) <= computed_radius and \
			...
			var computed_angle = fmod(rad2deg(centered_position.angle()+2.5*PI),360)
			var total_value = 0.0
...
func update_tooltip(data = null):
	...
	if data != null:
		if tooltip_data != data:
			set_tooltip('%s: %.02f%%' % [data.name, data.value*100])
			update_frame = true
...
func draw_line_chart():
	...
	draw_line(vertical_line[0], vertical_line[1], grid_color, 1.0) # y轴
...
func _make_custom_tooltip(for_text):
	var label = Label.new()
	label.text = for_text
	label.rect_scale = Vector2(10.0,10.0)
	return label
