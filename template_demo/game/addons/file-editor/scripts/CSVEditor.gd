...
onready var editor_columns_spacing: LineEdit = editor_settings.get_node(
	"EditorSettings/ColumnsSpacing/ColumnsSpacingLine"
)
...
func set_checked(key: TreeItem, to_check: bool) -> TreeItem:
	if key != null and key.is_editable(0):
		key.set_checked(0, to_check)
	return key
...
func open_csv_file(filepath: String, csv_delimiter: String = ";") -> void:
	if rows_count != 0 and columns_count != 0:
		clear_editor()
	...
	for column in range(1, columns_count + 1):
		for row in range(1, rows_count + 1):
			if Columns.get_child(column).get_child(row).is_connected("text_changed", self, "_on_editing_cell"):
				Columns.get_child(column).get_child(row).connect(
					"text_changed", self, "_on_editing_cell", [Vector2(column, row)]
				)
...
func _on_translation_accept() -> void:
	...
	if token in ["", " "]:
		...
		tw.interpolate_property(
			$TranslationDialog/TranslationContainer/AuthToken,
			"modulate",
			Color("#ffffff"),
			Color("#ff0000"),
			0.5,
	...
	var source_lang_idx: int = (
		langs.find(source_lang_menu.get_text())
		if source_lang_menu.get_selected() == -1
		else source_lang_menu.get_selected()
	)
...
func _on_edit_confirmed() -> void:
	...
	if drop_rows.is_visible():
		...
		for row in drop_rows.get_children():
			if row.is_pressed():
				rows_to_drop.append(int(row.get_text()))
	...
	if drop_columns.is_visible():
		...
		for column in drop_columns.get_children():
			if column.is_pressed():
				columns_to_drop.append(int(column.get_text()))
...
func _on_editor_settings_confirmed() -> void:
	for column in range(0, columns_count + 1):
		...
		if column == 0:
			continue
		Columns.get_child(column).get_child(1).set_custom_minimum_size(
			Vector2(float(editor_columns_length.get_text()), 0.0)
		)
...
func _on_less_pressed() -> void:
	if edit_columns.is_visible():
		if int(new_columns_line.get_text()) > 0:
			new_columns_line.set_text(str(int(new_columns_line.get_text()) - 1))
	if edit_rows.is_visible():
		if int(new_rows_line.get_text()) > 0:
